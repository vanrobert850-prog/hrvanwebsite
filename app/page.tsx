...
const artworks = [
    { id: 1, title: 'Golden horizon',  artist: 'Maria Ruiz',    country: 'Spain',        price: 1200, medium: 'Oil on canvas',     size: '24 × 36 in', cat: 'Paintings', img: 'https://images.unsplash.com/photo-1547826039-bfc35e0f1ea8?w=600&q=80' },
    { id: 2, title: 'Silent garden',   artist: 'James Lee',     country: 'United States', price: 850,  medium: 'Fine art print',    size: '18 × 24 in', cat: 'Prints',    img: 'https://images.unsplash.com/photo-1518998053901-5348d3961a04?w=600&q=80' },
    { id: 3, title: 'Urban dusk',      artist: 'Sofia Martens', country: 'Belgium',       price: 2100, medium: 'Acrylic on canvas', size: '40 × 50 in', cat: 'Paintings', img: 'https://images.unsplash.com/photo-1561214115-f2f134cc4912?w=600&q=80' },
    { id: 4, title: 'Morning bloom',   artist: 'Carlos Vega',   country: 'Mexico',        price: 650,  medium: 'Fine art print',    size: '16 × 20 in', cat: 'Prints',    img: 'https://images.unsplash.com/photo-1549490349-8643362247b5?w=600&q=80' },
    { id: 5, title: 'Desert wind',     artist: 'Layla Hassan',  country: 'Egypt',         price: 1750, medium: 'Oil on canvas',     size: '30 × 40 in', cat: 'Paintings', img: 'https://images.unsplash.com/photo-1578301978693-85fa9c0320b9?w=600&q=80' },
    { id: 6, title: 'Ocean whisper',   artist: 'Nina Storm',    country: 'Denmark',       price: 980,  medium: 'Fine art print',    size: '20 × 28 in', cat: 'Prints',    img: 'https://images.unsplash.com/photo-1541961017774-22349e4a1262?w=600&q=80' },
    { id: 7, title: 'Crimson fields',  artist: 'Andres Mora',   country: 'Colombia',      price: 3200, medium: 'Oil on canvas',     size: '48 × 60 in', cat: 'Paintings', img: 'https://images.unsplash.com/photo-1604871000636-074fa5117945?w=600&q=80' },
    { id: 8, title: 'Quiet forest',    artist: 'Yuki Tanaka',   country: 'Japan',         price: 720,  medium: 'Fine art print',    size: '14 × 20 in', cat: 'Prints',    img: 'https://images.unsplash.com/photo-1531913764164-f85c52e6e654?w=600&q=80' },
]

const artists = [
    { slug: 'maria-ruiz',    nameKey: 'Maria Ruiz',    specialtyKey: 'Oil paintings',         photo: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=200&q=80' },
    { slug: 'james-lee',     nameKey: 'James Lee',     specialtyKey: 'Fine art prints',       photo: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=200&q=80' },
    { slug: 'sofia-martens', nameKey: 'Sofia Martens', specialtyKey: 'Abstract paintings',    photo: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=200&q=80' },
    { slug: 'carlos-vega',   nameKey: 'Carlos Vega',   specialtyKey: 'Prints & illustration', photo: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&q=80' },
]
...
function ArtCard({ art, delay }: { art: typeof artworks[0]; delay: number }) {
    const ref = useReveal()
    return (
        <Link ref={ref as any} href={`/artwork/${art.id}`} className={`art-card reveal d${delay}`}
              aria-label={`${art.title} by ${art.artist}`}>
            <div className="art-card-img">
                <img src={art.img} alt={`${art.title} — ${art.medium} by ${art.artist}`} loading="lazy" width="400" height="533" />
                <div className="art-card-actions">
                    <button className="art-card-btn" aria-label="Save"><HeartSmIcon /></button>
                    <button className="art-card-btn" aria-label="Add to cart"><CartSmIcon /></button>
                </div>
            </div>
                ...
                <div style={{ display: 'flex', gap: '10px', flexShrink: 0 }}>
                    <button onClick={() => dismiss('declined')} style={{
                        ...
                        fontFamily: 'inherit', transition: 'all 0.25s ease',
                            ...
                            onMouseEnter={e => { (e.target as HTMLElement).style.borderColor = 'rgba(255,255,255,0.5)'; (e.target as HTMLElement).style.color = 'rgba(255,255,255,0.9)' }}
                    ...
                    <button onClick={() => dismiss('accepted')} style={{
                        ...
                        transition: 'all 0.25s ease',
                            ...
                            onMouseEnter={e => { (e.target as HTMLElement).style.background = '#9A4A2E' }}
                            onMouseLeave={e => { (e.target as HTMLElement).style.background = '#B85C38' }}
                    >
                        {t('cookie.accept')}
                    </button>
                </div>
...
export default function HomePage() {
    ...
    const filtered = activeFilter === 'all'      ? artworks
    ...
    return (
        <>
            ...
            <nav className="navbar" role="navigation" aria-label="Main navigation">
                <Link href="/" className="navbar-logo" aria-label="HR FineArt — Home">
                    <div className="logo-ring" aria-hidden="true"><div className="logo-dot" /></div>
                    HR FineArt
                </Link>
                ...
                <div className="navbar-right">
                    <div className="navbar-search" role="search">
                        <SearchIcon />
                        <input placeholder={t('nav.search')} aria-label={t('nav.search')} type="search" />
                    </div>
                    <button className="navbar-icon-btn" aria-label="Account"><UserIcon /></button>
                    <Link href="/cart" className="navbar-icon-btn" aria-label="Cart" style={{ position: 'relative' }}>
            ...
            <main>
                ...
                <section className="hero" aria-label="Featured artwork carousel">
                    ...
                    <div className="hero-arrows" aria-label="Carousel navigation">
                    ...
                    <div style={{ position: 'absolute', bottom: 20, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8, zIndex: 4 }}
                         role="tablist" aria-label="Carousel slides">
                        {heroSlides.map((_, i) => (
                            <button key={i} onClick={() => setHeroIndex(i)}
                                    role="tab" aria-selected={heroIndex === i} aria-label={`Slide ${i + 1}`}
                ...
                <section className="section-wrap" aria-labelledby="featured-heading">
                    ...
                    <div className="art-grid" role="list">
                        {filtered.slice(0, 4).map((art, i) => <ArtCard key={art.id} art={art} delay={i + 1} />)}
                    </div>
                </section>
                ...
                <section className="section-wrap" style={{ paddingTop: '56px' }} aria-labelledby="arrivals-heading">
                    ...
                    <div className="art-grid" role="list">
                        {filtered.slice(4, 8).map((art, i) => <ArtCard key={art.id} art={art} delay={i + 1} />)}
                    </div>
                </section>
                    ...
                    <div className="artists-grid">
                        {artists.map((a, i) => (
                            <Link key={a.slug} href={`/artist/${a.slug}`} className={`artist-card reveal d${i + 1}`}
                                  aria-label={`${a.nameKey} — ${a.specialtyKey}`}>
                                <img className="artist-photo" src={a.photo} alt={a.nameKey} loading="lazy" width="56" height="56" />
                                <p className="artist-name">{a.nameKey}</p>
                                <p className="artist-specialty">{a.specialtyKey}</p>
                            </Link>
                        ))}
                    </div>
                </section>
                ...
                <section className="testimonials-section" aria-labelledby="testimonials-heading">
                    ...
                    <blockquote className={`testimonial-card${fading ? ' fading' : ''}`}>
                        <div className="testimonial-img">
                            <img src={testimonials[activeDot].img} alt="Artwork" loading="lazy" width="120" height="120" />
                        </div>
                        <div>
                ...
                <section className="cta-banner" aria-labelledby="cta-heading">
                    <img className="cta-banner-img" src="https://images.unsplash.com/photo-1513519245088-0e12902e5a38?w=1600&q=80" alt="Art advisor" loading="lazy" width="1600" height="360" />
                ...
                <footer className="footer" role="contentinfo">
                    ...
                    <div className="footer-grid">
                        <div>
                            ...
                            <div className="footer-email-wrap" role="form" aria-label="Newsletter signup">
                                <input className="footer-email-input" placeholder={t('footer.email')} type="email" aria-label={t('footer.email')} />
                                <button className="footer-email-btn" aria-label="Subscribe"><SendIcon /></button>
                            </div>
                        </div>
